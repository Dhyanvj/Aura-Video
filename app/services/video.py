...
@lru_cache(maxsize=1)
def _load_bgm_catalog(song_dir: str) -> dict:
    # Mood/palette tags for resource/songs/*.mp3, generated by
    # scripts/classify_bgm.py (the raw mp3s carry no genre/mood metadata of
    # their own). Missing/unreadable is a normal, expected state - callers
    # must fall back to the unfiltered pool rather than fail a render.
    catalog_path = os.path.join(song_dir, "catalog.json")
    try:
...
def _bgm_files_for_palette(song_dir: str, palette: str) -> List[str]:
    catalog = _load_bgm_catalog(song_dir)
    matches = [
        os.path.join(song_dir, name)
        for name, tags in catalog.items()
        if tags.get("palette") == palette
    ]
    return [path for path in matches if os.path.isfile(path)]


def get_bgm_file(bgm_type: str = "random", bgm_file: str = "", palette: str = ""):
    if not bgm_type:
        return ""
    ...
    if bgm_type == "random":
        song_dir = utils.song_dir()
        files = _bgm_files_for_palette(song_dir, palette) if palette else []
        if not files:
...
def generate_video(
    ...
    bgm_file = get_bgm_file(bgm_type=params.bgm_type, bgm_file=params.bgm_file, palette=params.bgm_palette or "")
    if bgm_file:
        try:
