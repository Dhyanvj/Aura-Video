...
export default function Analytics() {
  ...
  if (!data) return <div className="text-slate-500 dark:text-slate-400">Loading...</div>;

  const budgetUsed = data.monthly_budget_cap_usd > 0 ? data.monthly_spend_usd / data.monthly_budget_cap_usd : 0;

  return (
    <div>
      <h1 className="mb-1 text-xl font-semibold text-slate-900 dark:text-slate-100">Analytics</h1>
      ...
      <div className="mb-6 grid grid-cols-3 gap-4">
        <div className="rounded-lg border border-border bg-panel p-4">
          <div className="text-2xl font-semibold text-slate-900 dark:text-slate-100">{data.videos.length}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400">Tracked / archived videos</div>
        </div>
        <div className="rounded-lg border border-border bg-panel p-4">
          <div className="text-2xl font-semibold text-slate-900 dark:text-slate-100">${data.monthly_spend_usd.toFixed(2)}</div>
          <div className="text-xs text-slate-500 dark:text-slate-400">Spend this month</div>
        </div>
        <div className="rounded-lg border border-border bg-panel p-4">
      ...
      <div className="flex flex-col gap-3">
        {data.videos.map((video) => (
          <div key={video.project_id} className="rounded-lg border border-border bg-panel p-4">
            <div className="mb-2 flex items-center justify-between">
              <span className="font-medium text-slate-900 dark:text-slate-100">{video.topic}</span>
            ...
            <div className="flex gap-4">
              {video.checkpoints.map((cp, i) => (
                <div key={i} className="rounded border border-border bg-panel2 p-2 text-xs">
                  <div className="mb-1 font-medium text-slate-600 dark:text-slate-300">{cp.checkpoint_hours}h checkpoint</div>
                  <div className="text-slate-500 dark:text-slate-400">
                    {cp.views} views &middot; {cp.likes} likes &middot; {cp.comments} comments
                  </div>
                  {cp.note && <div className="mt-1 text-slate-500 dark:text-slate-500">{cp.note}</div>}
                </div>
              ))}
        ...
        {data.videos.length === 0 && <p className="text-sm text-slate-500 dark:text-slate-500">No published videos being tracked yet.</p>}
      </div>
    </div>
  );
}
